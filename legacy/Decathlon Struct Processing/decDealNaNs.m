...
for i = 1:length(data)
    ...
    %}
        ...
        %NOTE: Special condition used to edit the matrix after already made
    if isempty(data(i).ymaze)==1
        data(i).ymaze2=nanData(i).ymaze;
    end
    
        nanData(i).circles.c2_mu = NaN;
        nanData(i).circles.c2_speed = NaN;
        nanData(i).circles.c2_edgeposition = NaN;
        nanData(i).circles.c2_habituation = NaN;
        %nanData(i).circles.numTrials = NaN;
    
    if isempty(data(i).circles2) == 1
        data(i).circles2=nanData(i).circles;
    end
    %{
