function stimProps=optoInitializeBlackWhiteStim(nRow,nCol,rect_W,rect_H)
...
blankStim=zeros(stimH,stimW);
stimCenter=round(stimW/2);
rightWhite=blankStim;
rightWhite(:,stimCenter+1:end)=1;
leftWhite=blankStim;
leftWhite(:,1:stimCenter)=1;
blankTex = Screen('MakeTexture', window, blankStim);
Rtex = Screen('MakeTexture', window, rightWhite);
Ltex = Screen('MakeTexture', window, leftWhite);
...
% Permute dstRects to match the maze
dx=windowRect(3)-dstRects(1,:);
dy=windowRect(4)-dstRects(2,:);
dy=dy.^2;
d=dx+dy;
[val,permutation]=sort(d);
dstRects=dstRects(:,permutation);
