...
gaborAngles = zeros(1,length(nGabors));
...
while ~KbCheck
    ...
    %Create random number array per gabor
    anglechange = rand(1,nGabors) <= threshold;
    
    %Update gaborAngles
    gaborAngles = gaborAngles + 120*anglechange;
        
        %account for angles greater than 360
        largegaborAngles = gaborAngles > 240;
        gaborAngles(largegaborAngles) = 0;
    ...
    Screen('DrawTextures', window, gabortex, [], allRects, gaborAngles,...
        [], [], [0.0 0.0 2.0 0.0], [], kPsychDontDoRotation, propertiesMat');
