...
corridorSize = 15; % in mm (formerly 8mm)
...
if doPlot
    ...
    for i = 1:flyTracks.nFlies
        flyTracks.probA(i) = nanmean(refOdorChosen{i}); % prop. total choices made towards refOdor
    end
    ...
    for k = 1:flyTracks.nFlies
        ...
        [ptch1, ptch2] = makePatches(flyTracks,k,corridorPos);
        ...
        if strcmp(refOdor, flyTracks.stim{4,1}(1))
            set(ptch1,'edgecolor','none','facecolor','m', 'faceAlpha', 0.3)
            set(ptch2,'edgecolor','none','facecolor','g', 'faceAlpha', 0.3)
            ...
            set(ptch1,'edgecolor','none','facecolor','g', 'faceAlpha', 0.3)
            set(ptch2,'edgecolor','none','facecolor','m', 'faceAlpha', 0.3)
            plot(flyTracks.centroidLocal(tmp(sideAexit{k}),2,k), flyTracks.etimes(tmp(sideAexit{k})), '*m')
            plot(flyTracks.centroidLocal(tmp(~sideAexit{k}),2,k), flyTracks.etimes(tmp(~sideAexit{k})), '*g')
        end
    ...
    %         % Label each track with p(choosing refOdor) and total n choices
    %         % during odor period
    %         text(250*ct + 70, -5, [sprintf('%0.2f', flyTracks.probA(i)) ' (' sprintf('%2.0f', length(refOdorChosen{i})) ')'])
    %     end
